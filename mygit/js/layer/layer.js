...
Class.pt.config = {
    ...
    iframe: {src: '/sentsin.com', scrolling: 'auto'},
    loading: {type: 0},
...
Class.pt.openLayer = function(){
    ...
    layer.autoArea = function(index){
  	...
  	layer.autoTypeOfOnePage=function(index,config){
	  	var times = index || that.index,
	  	config = $.extend({} , that.config , config);
	  	var page = config.page;
	    var layerE = $('#'+ doms[0] + times), layerTitle = layerE.find(doms[2]), layerMian = layerE.find(doms[5]);
	    var titHeight = config.title[1] ? layerTitle.innerHeight() : 0, outHeight, btnHeight = 0;
	    if(config.area[0] === 'auto' && layerMian.outerWidth() >= config.maxWidth){    
...
layer.area = function(index, options){
    ...
    if(type === ready.type[1] || type === ready.type[2]){
        ...
        options.width && options.height && main.css({width: options.width, height: options.height});
...
layer.restore = function(index){
    ...
    layero.offset({top:area[2],left:area[3]});
